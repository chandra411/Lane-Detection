    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        for i in range(unit):
            with tf.variable_scope('unit_'+str(i+1), reuse=tf.AUTO_REUSE):
                ...
                if i == 0:
                    if name != 'block1':
                        net, shortcut = bottleneck(net, channel, is_train,stride=2,shortcut_conv=True)
                    else:
                        net, shortcut = bottleneck(net, channel, is_train,stride=1,shortcut_conv=True)
                else:
...
def resnet(input_, resnet_v2, is_train):
    ...
    with tf.variable_scope(resnet_v2, reuse=tf.AUTO_REUSE):
        ...
        # convolution 1
        with tf.variable_scope('conv1', reuse=tf.AUTO_REUSE):
            w_avg_init = tf.get_variable(initializer=w_initializer,
                                     shape=[7, 7, 3, 64],
                                     name='weights', regularizer=regularizer,
                                     collections=['pretrain', key])#,trainable=False)

            # init_kernel =tf.reduce_mean(kernel,axis=2,name='init_kern')
            #stack_kernel =tf.stack([init_kernel] * 7,name = 'init_kern2')
            #_kernel=tf.reshape(stack_kernel,shape=[7,7,7,64],name='init_kern_reshape')
            # init_kernel = tf.expand_dims(init_kernel,axis=2,name='expand_dim_init_kernel')
            # stack_kernel =tf.stack([init_kernel] * 2,name = 'init_kern2')
            # w_avg_init = tf.concat([kernel,init_kernel],axis=2,name= 'w_avg_init_conv1')

            kernel_mod = tf.get_variable(initializer=w_avg_init,
                                     name='weights_input_channel', regularizer=regularizer,
                                     collections=['mod_pretrain', key])
            net = tf.nn.conv2d(input=input_, filter=kernel_mod, strides=[1, 2, 2, 1], padding='SAME')
        ...
        for i in range(4):
            net = block(net, 'block'+str(i+1), UNITS[resnet_v2][i],CHANNELS[i], is_train)
            
            print('block'+str(i+1), net)
            
            skip_conn_layer['block'+str(i+1)] = net

        net = tf.layers.batch_normalization(inputs=net, axis=-1,
                                            training=is_train, name='postnorm')
        net = tf.nn.relu(net)

    return net, skip_conn_layer
